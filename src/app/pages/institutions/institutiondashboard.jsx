...
const ReportIssuePage = ({ darkMode, institutionData, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (onSuccess) {
        onSuccess('Issue reported successfully! We will get back to you soon.');
      }
    } catch (err) {
      setError(err.message);
    } finally {
    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className={`p-6 rounded-2xl shadow-md ${darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
        <div className="mb-6">
          ...
          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            Let us know about any problems you're experiencing and we'll help resolve them as soon as possible.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Please describe the issue in detail. Include any error messages, steps to reproduce, and what you were trying to accomplish..."
              required
            />
          </div>
          ...
          <div className={`p-4 rounded-lg ${darkMode ? 'bg-blue-900/20 border border-blue-800' : 'bg-blue-50 border border-blue-200'}`}>
            <div className="flex items-start">
              <Info size={18} className={`mt-0.5 mr-3 flex-shrink-0 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`} />
              <div>
                <h4 className={`font-medium mb-1 ${darkMode ? 'text-white' : 'text-gray-800'}`}>What to include in your report</h4>
                <ul className={`text-sm space-y-1 ${darkMode ? 'text-blue-300' : 'text-blue-700'}`}>
                  <li>• Detailed description of the problem</li>
                  <li>• Steps to reproduce the issue</li>
                  <li>• Any error messages you received</li>
                  <li>• Screenshots or documents that might help</li>
                  <li>• Your contact information if you'd like a direct response</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              className="px-6 py-3 bg-teal-600 text-white rounded-lg font-medium hover:bg-teal-700 transition-colors flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isSubmitting}
            >
              {isSubmitting ? (
                <>
    ...
    <div className="space-y-6 md:space-y-8">
      <div className={`p-6 rounded-2xl shadow-md ${darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
        <div className="mb-6">
          ...
          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            Stay updated with the latest information about your institution and students.
          </p>
        </div>
...
// Main Dashboard Component - UPDATED with Report Issue navigation
const InstitutionDashboard = () => {
  ...
  const handleReportIssueSuccess = (message) => {
    setSuccessMessage(message);
    setShowSuccessNotification(true);
  };
        ...
        {sidebarOpen && (
          ...
          >
            ...
            <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
              ...
              />
              <NavItem icon={<Flag size={20}/>} label="Report Issue" pageName="report-issue" />
            </nav>
            ...
            >
              ...
              <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                ...
                <NavItem 
                  ...
                  badgeCount={unreadCount}
                />
                <NavItem icon={<Flag size={20}/>} label="Report Issue" pageName="report-issue" isMobile={true} />
              </nav>
              <div className={`p-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                <button 
                  onClick={toggleDarkMode} 
            ...
            <div className="flex items-center">
              ...
              <h1 className={`text-xl md:text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>
                {activePage === 'dashboard' ? 'Dashboard' : 
                 ...
                 activePage === 'report-issue' ? 'Report Issue' : 'Notifications'}
              </h1>
            </div>
